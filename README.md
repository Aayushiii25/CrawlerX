...
# Runs REST API and WebSocket hub on http://localhost:8080
```

### 3. Start Crawler Nodes
In separate terminal instances:
```bash
# Start Node 1
CRAWLERX_NODE_ID=node-1 CRAWLERX_WORKER_COUNT=5 go run ./cmd/crawler

# Start Node 2 (Optional)
CRAWLERX_NODE_ID=node-2 CRAWLERX_WORKER_COUNT=5 go run ./cmd/crawler
```
...
### 5. Seed Crawl
Submit URLs directly via the dashboard input bar, or seed via curl:
```bash
make seed
```
...
The Next.js dashboard can be easily deployed to **Vercel** to provide cloud-based monitoring. Since the frontend dashboard sits in a subdirectory, configure the deployment as follows:

### 1. Vercel Project Settings
1. Import your repository into the Vercel dashboard.
2. In the **Configure Project** step, set the **Root Directory** to `dashboard` (this ensures Vercel runs NPM commands inside the correct package workspace).
3. Keep the **Framework Preset** as **Next.js** (Vercel automatically detects this and configures the build command `next build` and output directory `.next`).

### 2. Environment Variables
In Vercel's **Environment Variables** section, configure the connection settings for your public coordinator service:

| Key | Value | Description |
|---|---|---|
| `NEXT_PUBLIC_API_URL` | `https://your-coordinator-domain.com` | Public HTTP API endpoint of the Coordinator |
| `NEXT_PUBLIC_WS_URL` | `wss://your-coordinator-domain.com/ws` | Public WebSocket endpoint for live log streaming |

*Note: Ensure you use `wss://` for secure WebSocket connections if your coordinator runs behind HTTPS.*

### 3. Coordinator CORS Configuration
When hosting the dashboard on Vercel, the backend coordinator will reject API requests unless its origin is authorized. Update the coordinator's environment variable `CRAWLERX_CORS_ORIGINS` on your hosting server (e.g., AWS, GCP, Fly.io) to include the Vercel production URL:

```bash
CRAWLERX_CORS_ORIGINS="http://localhost:3000,https://your-dashboard.vercel.app"
```
